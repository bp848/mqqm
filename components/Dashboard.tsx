...
// FIX: The chart component is now dynamically imported to avoid server-side rendering issues.
...
// Moved to components/charts/MonthlyTrendChart.tsx
// const MonthlyTrendChart: React.FC<{ data: any[] }> = ({ data }) => {
//     // FIX: Add hasMounted state to prevent recharts useContext error on initial render
...
const Dashboard: React.FC<DashboardProps> = ({ jobs, journalEntries, accountItems, pendingApprovalCount, onNavigateToApprovals }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <React.Suspense fallback={<div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm h-[350px] flex items-center justify-center"><Loader className="w-8 h-8 animate-spin"/></div>}>
