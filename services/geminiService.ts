...
// AI機能をグローバルに制御する環境変数
// Removed: const NEXT_PUBLIC_AI_OFF = getEnvValue('NEXT_PUBLIC_AI_OFF') === '1';

const API_KEY = getEnvValue('API_KEY') ?? getEnvValue('GEMINI_API_KEY');

// Removed: if (!API_KEY) { // Changed condition to directly check API_KEY
// Removed:   console.error("API_KEY environment variable not set. AI functions might be unavailable.");
// Removed: }

// Removed: const ai = API_KEY ? new GoogleGenAI({ apiKey: API_KEY }) : null;

const getAiClient = () => {
    const key = getEnvValue('API_KEY') ?? getEnvValue('GEMINI_API_KEY');
    if (!key) {
...
const checkOnlineAndAIOff = () => {
    // Removed: if (NEXT_PUBLIC_AI_OFF) {
    // Removed:     throw new Error('AI機能は現在無効です。');
    // Removed: }
    // Ensure API_KEY is accessed within this function to reflect latest config
    const currentApiKey = getEnvValue('API_KEY') ?? getEnvValue('GEMINI_API_KEY');
    if (!currentApiKey) { // Changed to directly use currentApiKey
        throw new Error('AI APIキーが設定されていません。');
    }
...
export const analyzeLeadData = async (leads: Lead[]): Promise<string> => {
    ...
    return withRetry(async () => {
        ...
        データサンプル:
        ${JSON.stringify(leads.slice(0, 3).map(l => ({ company: l.company, status: l.status, inquiryType: l.inquiryTypes, message: l.message })), null, 2)}
        `;
        const response = await ai.models.generateContent({ model, contents: prompt, config: { } });
...
Web検索も活用し、市場トレンドや競合情報も踏まえ、洞察に満ちた回答を生成してください。`;
